 ...
 */
public class SaveAndCloseEditor extends WebActionHandlerSupport {

    public final static String ID = "System.SaveAndCloseEditor";
    ...
    public WebActionResult execute(WebActionParameter parameter) throws ClientException {
        ...
        // Try to save
        super.executeAction(saveAction, parameter);

        // Close Tab if saved
        WorkItem currentItem = workArea.getCurrentItem();
        if (!currentItem.getModel().isDirty()) {

            workArea.closeItem(currentItem.getInstanceId());

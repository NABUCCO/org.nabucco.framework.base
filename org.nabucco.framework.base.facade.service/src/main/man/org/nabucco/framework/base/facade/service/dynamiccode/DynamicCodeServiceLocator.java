 ...
 */
public class DynamicCodeServiceLocator extends ServiceLocator {

    /** The Logger */
    private static final NabuccoLogger logger = NabuccoLoggingFactory.getInstance().getLogger(
            DynamicCodeServiceLocator.class);
     ...
     */
    public DynamicCodeService getDynamicCodeService(Context ctx) {
        try {
            Service service = super.locateService(DynamicCodeService.JNDI_NAME, ctx);
            return (DynamicCodeService) service;
        } catch (Exception e) {
            logger.warning(e, "Cannot locate DynamicCodeService, using a default implementation.");
            return new DynamicCodeServiceDefaultImpl();
        }
    }
}

 ...
 */
public class CodeForNameVisitor extends CodeFacadeVisitor<Code> {

    private String nameValue;

    /**
     * Creates a new {@link CodeForNameVisitor} instance.
     * 
     * @param name
     *            the code name
     */
    public CodeForNameVisitor(String name) {
        this.nameValue = name;
    }
    ...
    protected void visitCode(Code code) throws VisitorException {
        if (code.getName() != null && code.getName().getValue() != null) {
            if (code.getName().getValue().compareTo(this.nameValue) == 0) {
                setResult(code);
            }
        }
    }

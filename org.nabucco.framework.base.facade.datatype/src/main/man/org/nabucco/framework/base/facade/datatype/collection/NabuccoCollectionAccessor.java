...
public class NabuccoCollectionAccessor {
	...
	/**
	 * Adds an element to the list without adding it to the assigned or removing
	 ...
	 */
	public <E extends NType> boolean addWithoutAssignment(Collection<E> target,
			E source) {
	...
	/**
	 * Adds a list to the list without adding it to the assigned or removing
	 * from the removed list.
	 * 
	 * @param target
	 *            the target collection
	 ...
	 */
	public <E extends NType> boolean addAllWithoutAssignment(
			Collection<E> target, Collection<E> source) {
		if (target instanceof NabuccoList<?>) {
	...
	/**
	 * Replaces the old element in all lists (delegate, assigned, unassigned)
	 * with the new one.
	 * 
	 * @param target
	 *            the target collection
	 ...
	 */
	public <E extends NType> boolean replace(Collection<E> target,
			E oldElement, E newElement) {
		if (target instanceof NabuccoList<?>) {
	 ...
	 * 
	 * @return the assigned list or an empty list if the parent list is not of
	 *         type {@link NabuccoCollection}
	 */
	public <E extends NType> List<E> getAssignedList(Collection<E> collection) {
		if (collection instanceof NabuccoList<?>) {
			return Collections.unmodifiableList(new ArrayList<E>(
					((NabuccoList<E>) collection).getAssignedList()));
		}
	 ...
	 * 
	 * @return the unassigned list or an empty list if the parent list is not of
	 *         type {@link NabuccoCollection}
	 */
	public <E extends NType> List<E> getUnassignedList(Collection<E> collection) {
		if (collection instanceof NabuccoList<?>) {
			return Collections.unmodifiableList(new ArrayList<E>(
					((NabuccoList<E>) collection).getUnassignedList()));
		}
		return Collections.emptyList();
	}
	...
	/**
	 * Returns the current state of the collection. Only {@link NabuccoList}
	 * instances are considered.
	 * 
	 * @param list
	 *            the list to check
	 * 
	 * @return return the nabucco collection state or null if list is no
	 *         {@link NabuccoList}
	 */
	...
	/**
	 * Disconnects a collection from the database. Removes a persistent provider
	 * specific lazy initialized list and replaces it by an empty list.
	 * 
	 * @param collection
	 *            the collection to clean
	 */

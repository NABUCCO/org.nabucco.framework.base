 ...
 */
public class TitleBarExtensionParser extends NabuccoExtensionParserSupport implements ExtensionParser {

    private static final String ELEMENT_TITLEBAR = "titlebar";
    ...
    private static NabuccoLogger logger = NabuccoLoggingFactory.getInstance().getLogger(TitleBarExtensionParser.class);

    @Override
    public NabucoExtensionContainer parseExtension(Element element) throws ExtensionParserException {
        try {
            logger.debug("Parsing user interface titlebar configuration");

            TitleBarExtension extension = new TitleBarExtension();
            ExtensionId id = new ExtensionId(element.getAttribute(ATTR_ID));
            extension.setIdentifier(id);

            Element statusBarElement = super.getElementsByTagName(element, ELEMENT_TITLEBAR).get(0);

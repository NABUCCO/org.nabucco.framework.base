...
/**
 * Parser for the navigator area extension of the nabucco user interface.
 * 
 * @author Frank Ratschinski, PRODYNA AG
 */
public class NavigationAreaExtensionParser extends NabuccoExtensionParserSupport implements ExtensionParser {

    private static final String ELEMENT_NAVIGATIONAREA = "navigationarea";
    ...
    private static NabuccoLogger logger = NabuccoLoggingFactory.getInstance().getLogger(
            NavigationAreaExtensionParser.class);

    @Override
    public NabucoExtensionContainer parseExtension(Element element) throws ExtensionParserException {
        try {
            logger.debug("Parsing user interface navigator area configuration.");

            NavigationAreaExtension extension = new NavigationAreaExtension();
            ExtensionId id = new ExtensionId(element.getAttribute(ATTR_ID));
            extension.setIdentifier(id);

            Element navigatorAreaElement = super.getElementsByTagName(element, ELEMENT_NAVIGATIONAREA).get(0);

            extension.setLayout(super.getStringProperty(navigatorAreaElement, ATTR_LAYOUT));
